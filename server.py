    ...
    def run():
        ...
        try:
            ...
            if burp_cfg.get('enabled') and scout.session:
                try:
                    from core.burp_integration import BurpConfig
                    bc = BurpConfig(
...
# ── High-Confidence Vulnerabilities endpoint ───────────────────────────────
@app.route('/api/scan/<scan_id>/high-confidence')
def get_high_confidence_vulns(scan_id):
    """Return high-confidence vulnerability results as clean JSON."""
    if scan_id not in SCANS:
        return jsonify({'error': 'Not found'}), 404
    s = SCANS[scan_id]
    if s['status'] != 'complete':
    ...
    hc_vulns = results.get('high_confidence_vulns', [])
    
    # Return clean, structured data
    return jsonify({
        'scan_id': scan_id,
        'target': s['target'],
        'high_confidence_vulnerabilities': hc_vulns,
        'total_count': len(hc_vulns),
        'confidence_filter': 'HIGH_ONLY',
        'detection_method': '3_STAGE_DETECTION'
    })

# ── Technology Detection endpoint ───────────────────────────────────────────────
@app.route('/api/scan/<scan_id>/technology')
def get_technology(scan_id):
    """Return technology detection results as clean JSON."""
    if scan_id not in SCANS:
        return jsonify({'error': 'Not found'}), 404
    s = SCANS[scan_id]
    if s['status'] != 'complete':
    ...
    results = s.get('results', {})
    tech_data = results.get('technology_data', {})
    
    # Defensive: Ensure tech_data is always a dict
    if not isinstance(tech_data, dict):
        tech_data = {}
    
    # Return clean, structured technology data with safe defaults
    return jsonify({
        'server': tech_data.get('server', {}) or {},
        'backend': tech_data.get('backend', {}) or {},
        'frameworks': tech_data.get('frameworks', []) or [],
        'js_libraries': tech_data.get('js_libraries', []) or [],
        'third_party': tech_data.get('third_party', []) or [],
        'endpoints': tech_data.get('endpoints', []) or [],
        'external_domains': tech_data.get('external_domains', []) or []
    })
